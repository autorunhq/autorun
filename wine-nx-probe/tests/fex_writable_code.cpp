...
public:
  ...
  FEXCore::HLE::ExecutableRangeInfo QueryGuestExecutableRange(FEXCore::Core::InternalThreadState*, uint64_t address) override {
    return {address & ~4095ULL, 4096, (address & ~4095ULL) == 0x100000};
  }
...
int main(int argc, char** argv) {
  ...
  check(compiled != 0, "block not cached");
  for (unsigned i = 0; i < 10000; ++i) {
    ...
    check(thread->LookupCache->FindBlock(thread, 0x100000) == compiled, "data write recompiled unchanged code");
  }
