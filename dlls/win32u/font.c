...
static const WCHAR tahomaW[] =
    {'T','a','h','o','m','a',0};
#ifdef __SWITCH__
static void load_switch_font_substitutes(void)
{
    static const WCHAR ms_shell_dlgW[] =
        {'M','S',' ','S','h','e','l','l',' ','D','l','g',0};
    static const WCHAR ms_shell_dlg_2W[] =
        {'M','S',' ','S','h','e','l','l',' ','D','l','g',' ','2',0};
    WCHAR shell_fontW[LF_FACESIZE];
    const char *shell_font = "Tahoma";
    unsigned int i;

    for (i = 0; i < ARRAY_SIZE(nls_update_font_list); i++)
    {
        if (nls_update_font_list[i].ansi_cp != ansi_cp.CodePage ||
            nls_update_font_list[i].oem_cp != oem_cp.CodePage) continue;
        shell_font = nls_update_font_list[i].shelldlg;
        break;
    }

    asciiz_to_unicode( shell_fontW, shell_font );
    add_gdi_font_subst( ms_shell_dlgW, -1, shell_fontW, -1 );
    add_gdi_font_subst( ms_shell_dlg_2W, -1, tahomaW, -1 );
    nxfont_trace_wpath( "default_shell_font", shell_fontW, ansi_cp.CodePage, oem_cp.CodePage );
}
#endif
...
static void init_codepage_tables(void)
{
...
static void update_codepage( UINT screen_dpi )
{
    ...
    if (!wine_fonts_key)
    {
        nxfont_trace( "[NXFONT] codepage registry=0 ansi=%u oem=%u dpi=%u",
...
static struct gdi_font *select_font( LOGFONTW *lf, FMAT2 dcmat, BOOL can_use_bitmap )
{
...
#ifdef __SWITCH__
        nxfont_trace_wpath( "select_failed", lf->lfFaceName, lf->lfCharSet, 0 );
#endif
        return NULL;
    }
#ifdef __SWITCH__
    nxfont_trace_wpath( "select_request", lf->lfFaceName, lf->lfCharSet, csi.fs.fsCsb[0] );
    nxfont_trace_wpath( "select_family", face->family->family_name, face->fs.fsCsb[0], face->face_index );
    nxfont_trace_wpath( "select_file", face->file, face->flags, face->fs.fsCsb[0] );
#endif
    height = lf->lfHeight;
    ...
    {
#ifdef __SWITCH__
        nxfont_trace_wpath( "select_load_failed", face->file, lf->lfCharSet, face->fs.fsCsb[0] );
#endif
        free_gdi_font( font );
        return NULL;
    }
                                   ...
                                   BOOL ignore_rotation )
{
    ...
    {
        if (metrics)
            nxfont_trace( "[NXFONT] glyph hdc=%p ch=%04x fmt=%x size=%u ret=%08x box=%ux%u origin=%ld,%ld inc=%d,%d",
                          hdc, ch, format, size, ret, metrics->gmBlackBoxX, metrics->gmBlackBoxY,
                          (long)metrics->gmptGlyphOrigin.x, (long)metrics->gmptGlyphOrigin.y,
                          metrics->gmCellIncX, metrics->gmCellIncY );
        else
            nxfont_trace( "[NXFONT] glyph hdc=%p ch=%04x fmt=%x size=%u ret=%08x no_metrics",
                          hdc, ch, format, size, ret );
    }
#endif
    release_dc_ptr( dc );
    return ret;
...
static void load_directory_fonts( WCHAR *path, UINT flags )
{
    ...
    {
#ifdef __SWITCH__
        nxfont_trace( "[NXFONT] open_dir_skipped flags=%x", flags );
#endif
        return;
    }
...
#ifdef __SWITCH__
    nxfont_trace_wpath( "open_dir", path, flags, 0 );
#endif
    ...
    {
#ifdef __SWITCH__
        nxfont_trace_wpath( "open_dir_failed", path, flags, status );
#endif
        return;
    }
           ...
           io.Information)
    {
        ...
        for (;;)
        {
            if (!(info->FileAttributes & FILE_ATTRIBUTE_DIRECTORY))
            {
                ...
                count++;
#ifdef __SWITCH__
                if (count <= 24) nxfont_trace_wpath( "add_font", path, flags, 0 );
#endif
            }
    ...
    NtClose( handle );
#ifdef __SWITCH__
    nxfont_trace_wpath( "scan_done", path, flags, count );
#endif
}

#ifdef __SWITCH__
static BOOL switch_font_file_extension( const char *name )
{
    const char *ext = strrchr( name, '.' );

    if (!ext++) return FALSE;
    return !strcasecmp( ext, "fon" ) || !strcasecmp( ext, "otf" ) ||
           !strcasecmp( ext, "ttc" ) || !strcasecmp( ext, "ttf" );
}

static void load_switch_directory_fonts( const char *unix_dir, const char *nt_prefix, UINT flags )
{
    WCHAR path[MAX_PATH];
    struct dirent *entry;
    unsigned int count = 0;
    ...
    nxfont_trace( "[NXFONT] nx_open_dir flags=%x path=%s", flags, unix_dir );
    if (!(dir = opendir( unix_dir )))
    {
        nxfont_trace( "[NXFONT] nx_open_dir_failed flags=%x errno=%d path=%s", flags, errno, unix_dir );
        return;
    }
    ...
    while ((entry = readdir( dir )))
    {
        ...
        if (!switch_font_file_extension( entry->d_name )) continue;

        len = snprintf( full_path, sizeof(full_path), "%s/%s", unix_dir, entry->d_name );
        if (!len || len >= sizeof(full_path)) continue;
        if (stat( full_path, &st ) || S_ISDIR( st.st_mode )) continue;

        asciiz_to_unicode( path, nt_prefix );
        if (wcslen( path ) + strlen( entry->d_name ) + 1 >= MAX_PATH) continue;
        asciiz_to_unicode( path + wcslen(path), entry->d_name );

        font_funcs->add_font( path, flags );
        count++;
        if (count <= 24) nxfont_trace_wpath( "nx_add_font", path, flags, 0 );
    }
...
static unsigned int load_switch_shared_fonts(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(types); i++)
    {
        ...
        rc = plGetSharedFontByType( &font, types[i] );
        if (rc)
        {
            nxfont_trace( "[NXFONT] shared_font type=%u rc=%08x", types[i], rc );
            continue;
        }
        if (!font.address || !font.size)
        {
            nxfont_trace( "[NXFONT] shared_font type=%u empty addr=%p size=%u",
                          types[i], font.address, font.size );
            continue;
        }

        pthread_mutex_lock( &font_lock );
        added = font_funcs->add_mem_font( font.address, font.size, ADDFONT_ALLOW_BITMAP | ADDFONT_ADD_RESOURCE );
        pthread_mutex_unlock( &font_lock );

        total += added;
        nxfont_trace( "[NXFONT] shared_font type=%u size=%u added=%d total=%u",
                      types[i], font.size, added, total );
    }
...
static void load_file_system_fonts(void)
{
    ...
    if (wine_fonts_key && query_reg_ascii_value( wine_fonts_key, "Path", info, sizeof(value_buffer) ) &&
        info->Type == REG_SZ)
    {
        for (ptr = (WCHAR *)info->Data; ptr; ptr = next)
        {
            ...
            dir = get_nt_path( ptr );
            if (dir) load_directory_fonts( dir, ADDFONT_EXTERNAL_FONT );
            free( dir );
        }
    }
}
...
static void update_external_font_keys(void)
{
    ...
    if (!wine_fonts_key ||
        !(hkey = reg_create_key( wine_fonts_key, external_fontsW, sizeof(external_fontsW), 0, NULL )))
        return;
...
UINT font_init(void)
{
...
#ifdef __SWITCH__
    nxfont_trace( "[NXFONT] font_init start" );
#endif
    if ((hkcu_key = open_hkcu()))
    {
...
#ifdef __SWITCH__
        else nxfont_trace( "[NXFONT] wine_fonts_key=0" );
#endif
    }
#ifdef __SWITCH__
    else nxfont_trace( "[NXFONT] open_hkcu=0" );

    if (!dpi)
    {
        dpi = 96;
        nxfont_trace( "[NXFONT] dpi_fallback=%u", dpi );
    }
#else
    else return 0;
    if (!dpi) return 96;
    ...
    {
#ifdef __SWITCH__
        nxfont_trace( "[NXFONT] freetype_backend=0 dpi=%u", dpi );
#endif
        return dpi;
    }
#ifdef __SWITCH__
    nxfont_trace( "[NXFONT] freetype_backend=1 dpi=%u", dpi );
    load_switch_shared_fonts();
#endif
    ...
    if (!wine_fonts_key)
    {
        nxfont_trace( "[NXFONT] registry_cache=0" );
        load_switch_font_substitutes();
        reorder_font_list();
        load_system_links();
        dump_gdi_font_list();
        dump_gdi_font_subst();
