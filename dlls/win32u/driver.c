...
#ifdef __SWITCH__
        /* No external graphics driver on the Switch: plug the software
         * framebuffer display driver (dlls/win32u/winnx_drv.c) into the null
...
#ifdef __SWITCH__
/* Surfaces are created before pCreateWindow runs, so on the Switch the lazy
 * driver must trigger the real driver load here too — otherwise the very first
 * window gets a non-presenting offscreen surface instead of our framebuffer one. */
static BOOL loaderdrv_CreateWindowSurface( HWND hwnd, BOOL layered, const RECT *surface_rect,
                                           struct window_surface **surface )
{
    return load_driver()->pCreateWindowSurface( hwnd, layered, surface_rect, surface );
}
#endif
