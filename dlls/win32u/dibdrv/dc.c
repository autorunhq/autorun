...
static void nxwin_trace_surface_sample( const char *tag, struct window_surface *surface )
{
    char color_buf[FIELD_OFFSET( BITMAPINFO, bmiColors[256] )];
    BITMAPINFO *info = (BITMAPINFO *)color_buf;
    const DWORD *bits;
    int width, height;
    ...
    nxwin_trace( tag, (int)bits[0], (int)bits[(size_t)(height / 2) * width + width / 2],
                 (int)surface->bounds.right - (int)surface->bounds.left,
                 (int)surface->bounds.bottom - (int)surface->bounds.top );
}
...
#ifdef __SWITCH__
    if (nxwin_text_trace_count++ < 40)
    {
        nxwin_trace( "[NXWIN] exttext ret=%d x=%d y=%d flags=%x count=%u ch=%x",
                     ret, x, y, flags, count, str && count ? str[0] : 0 );
        nxwin_trace_surface_sample( "[NXWIN] after text p0=%08x pc=%08x bounds=%dx%d", physdev->surface );
    }
#endif
    unlock_surface( physdev );
    return ret;
...
#ifdef __SWITCH__
    if (nxwin_patblt_trace_count++ < 40)
    {
        nxwin_trace( "[NXWIN] patblt ret=%d dst=%d,%d %dx%d rop=%x",
                     ret, dst->x, dst->y, dst->width, dst->height, (int)rop );
        nxwin_trace_surface_sample( "[NXWIN] after pat p0=%08x pc=%08x bounds=%dx%d", physdev->surface );
    }
#endif
    unlock_surface( physdev );
    return ret;
