...
#include <sys/stat.h>
#include <fcntl.h>
#ifdef SONAME_LIBFREETYPE  /* dlfcn/mman only used by the freetype engine */
#include <dlfcn.h>
#ifndef __SWITCH__
#include <sys/mman.h>
#endif
...
static void *mmap( void *addr, size_t length, int prot, int flags, int fd, off_t offset )
{
    ...
    if (!(ptr = malloc( length ))) return MAP_FAILED;
    if (lseek( fd, offset, SEEK_SET ) == (off_t)-1) goto failed;

    while (done < length)
    {
        ssize_t ret = read( fd, ptr + done, length - done );
...
static void nxft_trace( const char *fmt, ... )
{
...
static void nxft_trace_wpath( const char *tag, const WCHAR *path, int ret )
{
    char pathA[180];
    unsigned int i;
    ...
    pathA[i] = 0;
    nxft_trace( "[NXFONT] %s ret=%d path=%s", tag, ret, pathA );
}
#endif
...
static BOOL is_symbol_charmap( FT_CharMap charmap )
{
    return charmap &&
           (charmap->encoding == FT_ENCODING_MS_SYMBOL ||
            (charmap->platform_id == TT_PLATFORM_MICROSOFT &&
             charmap->encoding_id == TT_MS_ID_SYMBOL_CS));
}
...
static inline void get_fontsig( FT_Face ft_face, FONTSIGNATURE *fs )
{
    ...
    if (fs->fsCsb[0] == 0)
    {
        ...
        {
            if (is_symbol_charmap( ft_face->charmaps[i] ))
            {
                fs->fsCsb[0] |= FS_SYMBOL;
                continue;
            }
                                           ...
                                           UINT face_index, UINT flags )
{
    ...
    if (unix_name)
    {
        ...
        {
#ifdef __SWITCH__
            nxft_trace( "[NXFONT] open_font_failed path=%s", unix_name );
#endif
            return NULL;
        }
        if (fstat( fd, &st ) == -1)
        {
            close( fd );
#ifdef __SWITCH__
            nxft_trace( "[NXFONT] stat_font_failed path=%s", unix_name );
#endif
            return NULL;
        }
        ...
        {
#ifdef __SWITCH__
            nxft_trace( "[NXFONT] map_font_failed path=%s size=%u", unix_name, (unsigned int)data_size );
#endif
            return NULL;
        }
#ifdef __SWITCH__
        nxft_trace( "[NXFONT] map_font_ok path=%s size=%u", unix_name, (unsigned int)data_size );
#endif
    }
...
#ifdef __SWITCH__
    if (This && This->family_name)
        nxft_trace( "[NXFONT] face_ok path=%s family0=%04x faces=%u scalable=%d fs=%08x",
                    unix_name ? unix_name : "<mem>", This->family_name[0], This->num_faces,
                    This->scalable, (unsigned int)This->fs.fsCsb[0] );
    else
        nxft_trace( "[NXFONT] face_fail path=%s", unix_name ? unix_name : "<mem>" );
#endif
                          ...
                          DWORD face_index, DWORD flags, DWORD *num_faces )
{
    ...
    {
#ifdef __SWITCH__
        nxft_trace( "[NXFONT] add_unix_face failed path=%s face=%u flags=%x",
                    unix_name ? unix_name : "<mem>", (unsigned int)face_index, (unsigned int)flags );
#endif
        return 0;
    }
...
#ifdef __SWITCH__
    nxft_trace_wpath( unixname ? "unix_path_ok" : "unix_path_fail", file, unixname ? 1 : 0 );
    if (unixname) nxft_trace( "[NXFONT] unix_path=%s flags=%x", unixname, flags );
#endif
    if (unixname)
    {
...
#ifdef __SWITCH__
    nxft_trace_wpath( "add_font_ret", file, ret );
#endif
    return ret;
}
...
#ifdef __SWITCH__
    nxft_trace( "[NXFONT] freetype_init version=%d.%d.%d", FT_Version.major, FT_Version.minor, FT_Version.patch );
#endif
...
static struct font_mapping *map_font_file( const char *name )
{
    ...
    LIST_FOR_EACH_ENTRY( mapping, &mappings_list, struct font_mapping, entry )
    {
...
#endif
        {
            ...
            close( fd );
#ifdef __SWITCH__
            nxft_trace( "[NXFONT] mapping_cache_hit path=%s size=%u", name,
...
#ifdef __SWITCH__
    if (!(mapping->name = strdup( name )))
    {
        munmap( mapping->data, st.st_size );
        free( mapping );
        return NULL;
    }
#endif
...
#ifdef __SWITCH__
    nxft_trace( "[NXFONT] mapping_cache_new path=%s size=%u dev=%llu ino=%llu", name,
                (unsigned int)mapping->size, (unsigned long long)mapping->dev,
                (unsigned long long)mapping->ino );
#endif
    return mapping;
...
static BOOL select_charmap(FT_Face ft_face, FT_Encoding encoding)
{
    ...
    for (i = 0; i < ft_face->num_charmaps; i++)
    {
        BOOL matches = encoding == FT_ENCODING_MS_SYMBOL
                       ? is_symbol_charmap( ft_face->charmaps[i] )
                       : ft_face->charmaps[i]->encoding == encoding;
...
static BOOL freetype_load_font( struct gdi_font *font )
{
    ...
    if (font->file[0])
    {
        ...
        {
#ifdef __SWITCH__
            nxft_trace_wpath( "load_font_unix_path", font->file, 1 );
            nxft_trace( "[NXFONT] load_font_unix=%s", filename );
#endif
...
#ifdef __SWITCH__
            nxft_trace_wpath( "load_font_map_failed", font->file, 0 );
#endif
            return FALSE;
        }
    ...
    {
#ifdef __SWITCH__
        nxft_trace_wpath( "load_font_new_face_failed", font->file, 0 );
#endif
        return FALSE;
    }
...
#ifdef __SWITCH__
    encoding = pick_charmap( ft_face, font->charset );
    nxft_trace( "[NXFONT] load_charmap charset=%u encoding=%08x active=%08x platform=%u id=%u symbol=%u",
                font->charset, (unsigned int)encoding,
                ft_face->charmap ? (unsigned int)ft_face->charmap->encoding : 0,
                ft_face->charmap ? ft_face->charmap->platform_id : 0,
                ft_face->charmap ? ft_face->charmap->encoding_id : 0,
                is_symbol_charmap( ft_face->charmap ));
    nxft_trace_wpath( "load_font_ok", font->file, 1 );
#else
...
static BOOL freetype_get_glyph_index( struct gdi_font *font, UINT *glyph, BOOL use_encoding )
{
    ...
    if (font->charset == SYMBOL_CHARSET || is_symbol_charmap( ft_face->charmap ))
    {
