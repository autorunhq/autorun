...
#include <assert.h>
#include <pthread.h>
#ifdef SONAME_LIBEGL  /* dlfcn only used to dlopen the host EGL/GL driver */
#include <dlfcn.h>
#endif
#include <unistd.h>
...
/* The framebuffer-surface drawable helpers below are defined inside the EGL
 * block upstream but referenced unconditionally further down; without EGL no
 * framebuffer drawable is ever created, so these stubs keep those references
 * resolvable and the funcs-pointer comparisons permanently false. */
struct framebuffer_surface { struct opengl_drawable *target; };
static const struct opengl_drawable_funcs framebuffer_surface_funcs;
static struct framebuffer_surface *framebuffer_from_opengl_drawable( struct opengl_drawable *base ) { (void)base; return NULL; }
static struct opengl_drawable *framebuffer_surface_create( int format, struct client_surface *client, struct opengl_drawable *target ) { (void)format; (void)client; (void)target; return NULL; }
static BOOL needs_framebuffer_surface( HWND hwnd ) { (void)hwnd; return FALSE; }
