                              ...
                              NTSTATUS status, ULONG ret_len )
{
    ...
    snprintf( buf, sizeof(buf),
              ...
              params->ansi, params->ansi_dst, (unsigned int)status, ret_len );
    wine_nx_runtime_trace( buf );
}

static void nx_trace_mouse( const char *stage, const MSG *msg, INT hittest,
                            const GUITHREADINFO *info, UINT output_msg, BOOL dropped )
{
    char buf[224];
    ...
    snprintf( buf, sizeof(buf),
              ...
              (long)msg->pt.x, (long)msg->pt.y, info->hwndActive, info->hwndCapture, dropped );
    wine_nx_runtime_trace( buf );
}
#endif
...
#ifdef __SWITCH__
/* ntdll's: the page is not always at 0x7ffe0000 on Horizon (virtual_alloc_first_teb) */
...
#ifdef __SWITCH__
    nx_trace_winproc( "init", params, STATUS_SUCCESS, 0 );
#endif
    return TRUE;
}
...
#ifdef __SWITCH__
    nx_trace_winproc( "return", params, status, ret_len );
#endif
...
/***********************************************************************
 *           get_send_message_flags
 */
UINT get_send_message_flags(void)
{
    struct user_thread_info *thread_info = get_user_thread_info();
    return thread_info->receive_info ? thread_info->receive_info->flags : ISMEX_NOSEND;
}
...
{
    struct user_thread_info *thread_info = get_user_thread_info();
    INPUT_MESSAGE_SOURCE prev_source;
    ...
    {
        offset = (offset + 15) & ~15;
        if (!(params = malloc( offset + packed_size ))) goto done;
    }
...
#ifdef __SWITCH__
    nx_trace_mouse( "hit", msg, hittest, &info, msg->message, !msg->hwnd );
#endif

    if (!msg->hwnd || !is_current_thread_window( msg->hwnd ))
    {
#ifdef __SWITCH__
        nx_trace_mouse( "reject", msg, hittest, &info, msg->message, TRUE );
#endif
...
#ifdef __SWITCH__
    nx_trace_mouse( "dispatch", msg, hittest, &info, message, eat_msg );
#endif
    msg->message = message;
    return !eat_msg;
}
...
#ifdef __SWITCH__
    return counter.QuadPart * 120 / freq.QuadPart;
#else
...
#ifdef __SWITCH__
        /* Horizon has no event-backed driver fd yet.  GetMessage already
         * wakes periodically, so poll the libnx input source directly. */
...
BOOL WINAPI NtUserGetMessage( MSG *msg, HWND hwnd, UINT first, UINT last )
{
    ...
    {
#ifdef __SWITCH__
        LARGE_INTEGER delay = {.QuadPart = -100000}; /* poll every 10 ms until Horizon queues are event-backed */
