...
static inline void wine_nx_restore_pe_teb(void)
{
#if defined(__aarch64__)
    if (!NtCurrentTeb() && wine_nx_pe_teb)
        __asm__ volatile( "mov x18, %0" :: "r"(wine_nx_pe_teb) : "x18", "memory" );
#endif
}
#else
static inline void wine_nx_restore_pe_teb(void)
...
{
#if defined(__SWITCH__) || defined(__WINE_PE_BUILD)
    char buf[256];
    va_list args;
...
static const char *wine_nx_us_ascii( const UNICODE_STRING *s, char *buf, size_t size )
{
    unsigned int i, len;

    buf[0] = 0;
    if (!s || !s->Buffer) return buf;
    len = s->Length / sizeof(WCHAR);
    for (i = 0; i < len && i + 1 < size; i++)
    {
        WCHAR ch = s->Buffer[i];
        buf[i] = ch < 0x80 ? (char)ch : '?';
    }
    buf[i] = 0;
    return buf;
}
...
#ifdef __SWITCH__
static WINE_MODREF *wine_nx_pe_ntdll_module;
static char wine_nx_last_import_dll[256];
static NTSTATUS wine_nx_last_import_status;
...
{
    return wine_nx_last_import_dll;
}

NTSTATUS wine_nx_loader_last_import_status(void)
{
    return wine_nx_last_import_status;
}

const char *wine_nx_loader_last_open_path(void)
{
    return wine_nx_last_open_path;
}

NTSTATUS wine_nx_loader_last_open_status(void)
{
    return wine_nx_last_open_status;
}

const char *wine_nx_loader_last_export_diag(void)
{
    return wine_nx_export_diag;
}

static void wine_nx_record_import_failure( const char *name, NTSTATUS status )
{
    if (!name) name = "";
    snprintf( wine_nx_last_import_dll, sizeof(wine_nx_last_import_dll), "%s", name );
    wine_nx_last_import_status = status;
}

static void wine_nx_record_export_failure( const char *dll, const char *name )
{
    if (!dll) dll = "";
    if (!name) name = "";
    snprintf( wine_nx_last_import_dll, sizeof(wine_nx_last_import_dll), "%s!%s", dll, name );
    wine_nx_last_import_status = STATUS_ENTRYPOINT_NOT_FOUND;
}

static void wine_nx_record_open_status( const UNICODE_STRING *nt_name, NTSTATUS status )
{
    unsigned int i, len, out = 0;

    wine_nx_last_open_status = status;
    if (!nt_name || !nt_name->Buffer)
    {
        wine_nx_last_open_path[0] = 0;
        return;
    }

    len = nt_name->Length / sizeof(WCHAR);
    for (i = 0; i < len && out + 1 < sizeof(wine_nx_last_open_path); i++)
    {
        WCHAR ch = nt_name->Buffer[i];
        wine_nx_last_open_path[out++] = ch < 0x80 ? (char)ch : '?';
    }
    wine_nx_last_open_path[out] = 0;
}

static BOOL wine_nx_is_crt_apiset_dll( const WCHAR *libname )
{
    static const WCHAR prefix[] = L"api-ms-win-crt-";
    static const WCHAR suffix[] = L".dll";
    size_t suffix_len = ARRAY_SIZE(suffix) - 1;
    size_t len = wcslen( libname );

    if (wcsnicmp( libname, prefix, ARRAY_SIZE(prefix) - 1 )) return FALSE;
    return len > suffix_len && !_wcsicmp( libname + len - suffix_len, suffix );
}

static BOOL wine_nx_is_apiset_dll( const WCHAR *libname )
{
    return !wcsnicmp( libname, L"api-", 4 ) || !wcsnicmp( libname, L"ext-", 4 );
}

static char wine_nx_ascii_tolower_char( char ch )
{
    return (ch >= 'A' && ch <= 'Z') ? ch - 'A' + 'a' : ch;
}

static int wine_nx_ascii_strnicmp( const char *left, const char *right, size_t len )
{
    while (len--)
    {
        char a = wine_nx_ascii_tolower_char( *left++ );
        char b = wine_nx_ascii_tolower_char( *right++ );

        if (a != b || !a || !b) return (unsigned char)a - (unsigned char)b;
    }
    return 0;
}

static BOOL wine_nx_is_crt_apiset_name( const char *name )
{
    ...
    if (wine_nx_ascii_strnicmp( name, prefix, sizeof(prefix) - 1 )) return FALSE;
    return len > suffix_len && !_stricmp( name + len - suffix_len, suffix );
}
#endif
...
__ASM_GLOBAL_FUNC(call_dll_entry_point,
                  ...
                  "ret" )
#elif defined(__SWITCH__) && defined(__aarch64__)
                                         ...
                                         UINT reason, void *reserved )
{
    ...
    __asm__ volatile(
        ...
        "mov %[ret], x0\n\t"
        : [ret] "=r"(ret)
        : [func] "r"(proc), [teb] "r"(teb), [module] "r"(module),
          [reason] "r"(reason_ptr), [reserved] "r"(reserved)
...
#ifdef __SWITCH__
        if (!(mod->ldr.Flags & LDR_REDIRECTED)
#else
...
{
    static const struct file_id zero_id;
    LIST_ENTRY *mark, *entry;

    /* An all-zero object id is not a real file identity (e.g. filesystems
     * that do not provide inodes return zeroes). Treating it as one aliases
     * every such file to the first zero-id module. */
    if (!memcmp( id, &zero_id, sizeof(*id) )) return NULL;
...
#ifdef __SWITCH__
    if (!wine_nx_is_apiset_dll( buffer )) return STATUS_SUCCESS;
#endif
...
static int find_name_in_exports( HMODULE module, const IMAGE_EXPORT_DIRECTORY *exports, const char *name )
{
    ...
    {
        char *ename = get_rva( module, names[i] );
        if (!strcmp( ename, name )) return ordinals[i];
    }
#endif
    return -1;
}
...
static char wine_nx_dos_path_char( WCHAR ch )
{
    if (ch >= L'A' && ch <= L'Z') return (char)(ch - L'A' + L'a');
    if (ch == L'\\') return '/';
    if (ch < 0x80) return (char)ch;
    return '?';
}

static BOOL wine_nx_copy_ascii_wstr( const WCHAR *src, ULONG len, char *dst, size_t size, BOOL lower )
{
    size_t out = 0;
    ULONG i;
    ...
    for (i = 0; i < len; i++)
    {
        char ch = lower ? wine_nx_dos_path_char( src[i] ) : (src[i] < 0x80 ? (char)src[i] : '?');

        if (!ch) break;
        if (ch == '\\') ch = '/';
        if (out + 1 >= size) return FALSE;
        if (ch == '/' && out && dst[out - 1] == '/') continue;
        dst[out++] = ch;
    }
    dst[out] = 0;
    return TRUE;
...
static BOOL wine_nx_nt_name_to_sdmc_path( const UNICODE_STRING *nt_name, char *path, size_t size )
{
    const WCHAR *src;
    const char *root;
    ULONG len, i;
    ...
    if (!nt_name || !nt_name->Buffer || !size) return FALSE;
    ...
    if (len >= 5 &&
        ...
        (src[3] == L'x' || src[3] == L'X') &&
        (src[4] == L'/' || src[4] == L'\\'))
        return wine_nx_copy_ascii_wstr( src + 5, len - 5, path, size, FALSE );
    ...
    memcpy( path, root, out + 1 );

    for (i = 2; i < len; i++)
    {
        char ch = wine_nx_dos_path_char( src[i] );

        if (!ch) break;
        if (ch == '/' && out && path[out - 1] == '/') continue;
        if (out + 1 >= size) return FALSE;
        path[out++] = ch;
    }
    path[out] = 0;
...
static BOOL wine_nx_nt_name_basename( const UNICODE_STRING *nt_name, char *name, size_t size )
{
    const WCHAR *src;
    ULONG len, i, start = 0;
    size_t out = 0;
...
static FILE *wine_nx_file_exports_fopen( const UNICODE_STRING *nt_name, char *path, size_t size )
{
    ...
    if (wine_nx_nt_name_to_sdmc_path( nt_name, path, size ) && (file = fopen( path, "rb" )))
        return file;

    if (!wine_nx_nt_name_basename( nt_name, name, sizeof(name) )) return NULL;

    for (i = 0; i < ARRAY_SIZE(roots); i++)
    {
        int ret = snprintf( path, size, "%s/%s", roots[i], name );

        if (ret <= 0 || (size_t)ret >= size) continue;
...
static void wine_nx_file_exports_close( struct wine_nx_file_exports *ctx )
{
    if (ctx->data) RtlFreeHeap( GetProcessHeap(), 0, ctx->data );
    memset( ctx, 0, sizeof(*ctx) );
}

static BOOL wine_nx_file_rva_to_offset( const struct wine_nx_file_exports *ctx, DWORD rva, DWORD *offset )
{
    const IMAGE_SECTION_HEADER *section = IMAGE_FIRST_SECTION( ctx->nt );
    DWORD header_size;
    WORD i;

    if (ctx->nt->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC)
        header_size = ((const IMAGE_NT_HEADERS64 *)ctx->nt)->OptionalHeader.SizeOfHeaders;
    else if (ctx->nt->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC)
        header_size = ((const IMAGE_NT_HEADERS32 *)ctx->nt)->OptionalHeader.SizeOfHeaders;
    else
        return FALSE;
    ...
    for (i = 0; i < ctx->nt->FileHeader.NumberOfSections; i++)
    {
        ...
        if (span < section[i].SizeOfRawData) span = section[i].SizeOfRawData;
        if (!span || rva < va || rva - va >= span) continue;

        delta = rva - va;
        if (delta >= section[i].SizeOfRawData) return FALSE;
        file_offset = (ULONGLONG)section[i].PointerToRawData + delta;
        if (file_offset > 0xffffffffu) return FALSE;
        *offset = file_offset;
        return TRUE;
    }
    return FALSE;
...
static const void *wine_nx_file_ptr_from_rva( const struct wine_nx_file_exports *ctx, DWORD rva, size_t size )
{
    DWORD offset;

    if (!wine_nx_file_rva_to_offset( ctx, rva, &offset )) return NULL;
    if ((size_t)offset > ctx->size || size > ctx->size - (size_t)offset) return NULL;
    return (const char *)ctx->data + offset;
}

static const void *wine_nx_file_table_from_rva( const struct wine_nx_file_exports *ctx,
                                                DWORD rva, DWORD count, size_t elem_size )
{
    size_t bytes;

    if (elem_size && count > (size_t)-1 / elem_size) return NULL;
    bytes = (size_t)count * elem_size;
    return wine_nx_file_ptr_from_rva( ctx, rva, bytes );
}

static BOOL wine_nx_file_string_equals( const struct wine_nx_file_exports *ctx, DWORD rva, const char *name )
{
    size_t i;
    ...
    for (i = 0;; i++)
    {
        ...
        if (i > 0xffffffffu - rva) return FALSE;
        if (!(ch = wine_nx_file_ptr_from_rva( ctx, rva + i, 1 ))) return FALSE;
        if (*ch != name[i]) return FALSE;
        if (!*ch) return TRUE;
    }
}

static BOOL wine_nx_file_copy_string( const struct wine_nx_file_exports *ctx, DWORD rva, char *buffer, size_t size )
{
    size_t i;
...
static BOOL wine_nx_file_export_result( const struct wine_nx_file_exports *ctx, DWORD func_rva,
                                        DWORD *rva, char *forward, size_t forward_size )
{
    if (!func_rva) return FALSE;
    if (forward_size) forward[0] = 0;

    if (ctx->export_size && func_rva >= ctx->export_rva &&
        func_rva - ctx->export_rva < ctx->export_size)
    {
        if (!wine_nx_file_copy_string( ctx, func_rva, forward, forward_size )) return FALSE;
        *rva = 0;
        return TRUE;
    }
...
static BOOL wine_nx_file_find_named_export( const struct wine_nx_file_exports *ctx, const char *name,
                                            WORD hint, DWORD *rva, char *forward, size_t forward_size )
{
    const IMAGE_EXPORT_DIRECTORY *exports = ctx->exports;
    const DWORD *functions, *names;
    const WORD *ordinals;
    DWORD i;

    functions = wine_nx_file_table_from_rva( ctx, exports->AddressOfFunctions,
                                             exports->NumberOfFunctions, sizeof(*functions) );
    names = wine_nx_file_table_from_rva( ctx, exports->AddressOfNames,
                                         exports->NumberOfNames, sizeof(*names) );
    ordinals = wine_nx_file_table_from_rva( ctx, exports->AddressOfNameOrdinals,
                                            exports->NumberOfNames, sizeof(*ordinals) );
    if (!functions || !names || !ordinals) return FALSE;

    if (hint < exports->NumberOfNames && wine_nx_file_string_equals( ctx, names[hint], name ))
    {
        WORD ordinal = ordinals[hint];

        if (ordinal >= exports->NumberOfFunctions) return FALSE;
        return wine_nx_file_export_result( ctx, functions[ordinal], rva, forward, forward_size );
    }
    ...
    for (i = 0; i < exports->NumberOfNames; i++)
    {
        ...
        if (!wine_nx_file_string_equals( ctx, names[i], name )) continue;
        ordinal = ordinals[i];
        if (ordinal >= exports->NumberOfFunctions) return FALSE;
        return wine_nx_file_export_result( ctx, functions[ordinal], rva, forward, forward_size );
    }
...
static BOOL wine_nx_file_find_ordinal_export( const struct wine_nx_file_exports *ctx, DWORD ordinal,
                                              DWORD *rva, char *forward, size_t forward_size )
{
    const IMAGE_EXPORT_DIRECTORY *exports = ctx->exports;
    const DWORD *functions;
    ...
    if (!(functions = wine_nx_file_table_from_rva( ctx, exports->AddressOfFunctions,
                                                   exports->NumberOfFunctions, sizeof(*functions) )))
        return FALSE;
    return wine_nx_file_export_result( ctx, functions[index], rva, forward, forward_size );
}

static BOOL wine_nx_file_exports_open( const UNICODE_STRING *nt_name, struct wine_nx_file_exports *ctx )
{
    const IMAGE_DOS_HEADER *dos;
    const IMAGE_DATA_DIRECTORY *dir = NULL;
    const IMAGE_NT_HEADERS *nt;
    IMAGE_EXPORT_DIRECTORY *exports;
    char path[768];
    FILE *file;
    long end;
    size_t nt_offset, min_nt_size, sections_offset, image_size, read_size;

    memset( ctx, 0, sizeof(*ctx) );
    if (!(file = wine_nx_file_exports_fopen( nt_name, path, sizeof(path) ))) return FALSE;
    if (fseek( file, 0, SEEK_END ) || (end = ftell( file )) <= 0 || fseek( file, 0, SEEK_SET ))
    {
        fclose( file );
        return FALSE;
    }
    ...
    ctx->size = end;
    if (!(ctx->data = RtlAllocateHeap( GetProcessHeap(), 0, ctx->size )))
    {
        fclose( file );
        return FALSE;
    }

    read_size = fread( ctx->data, 1, ctx->size, file );
    fclose( file );
    if (read_size != ctx->size) goto failed;

    if (ctx->size < sizeof(*dos)) goto failed;
    dos = ctx->data;
    if (dos->e_magic != IMAGE_DOS_SIGNATURE || (LONG)dos->e_lfanew < 0) goto failed;
    nt_offset = dos->e_lfanew;
    min_nt_size = sizeof(DWORD) + sizeof(IMAGE_FILE_HEADER);
    if (nt_offset > ctx->size || min_nt_size > ctx->size - nt_offset) goto failed;

    nt = (const IMAGE_NT_HEADERS *)((const char *)ctx->data + nt_offset);
    if (nt->Signature != IMAGE_NT_SIGNATURE) goto failed;
    image_size = min_nt_size + nt->FileHeader.SizeOfOptionalHeader;
    if (image_size < min_nt_size || image_size > ctx->size - nt_offset) goto failed;

    sections_offset = nt_offset + image_size;
    if (nt->FileHeader.NumberOfSections >
        (ctx->size - sections_offset) / sizeof(IMAGE_SECTION_HEADER))
        goto failed;
    ...
    if (nt->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC)
    {
        ...
        if (nt64->OptionalHeader.NumberOfRvaAndSizes <= IMAGE_DIRECTORY_ENTRY_EXPORT) goto failed;
    ...
    else if (nt->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC)
    {
        ...
        if (nt32->OptionalHeader.NumberOfRvaAndSizes <= IMAGE_DIRECTORY_ENTRY_EXPORT) goto failed;
        dir = &nt32->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT];
    }
    else goto failed;

    if (!dir->VirtualAddress || dir->Size < sizeof(*exports)) goto failed;
    ctx->export_rva = dir->VirtualAddress;
    ctx->export_size = dir->Size;
    if (!(exports = (IMAGE_EXPORT_DIRECTORY *)wine_nx_file_ptr_from_rva( ctx, ctx->export_rva, sizeof(*exports) )))
        goto failed;
    ctx->exports = exports;
    return TRUE;

failed:
    wine_nx_file_exports_close( ctx );
    return FALSE;
}

static BOOL wine_nx_ensure_file_exports( WINE_MODREF *wm, struct wine_nx_file_exports *ctx, BOOL *opened )
{
    if (!*opened)
    {
        *opened = TRUE;
        return wine_nx_file_exports_open( &wm->ldr.FullDllName, ctx );
    }
    return ctx->data != NULL;
}

static void wine_nx_record_export_diag( const char *name, const WINE_MODREF *wm )
{
    const IMAGE_NT_HEADERS *nt;
    void *base = wm ? wm->ldr.DllBase : NULL;
    char full[512], sdmc[640];
    DWORD exp_va = 0, exp_sz = 0, nrva = 0;
    unsigned int i, flen;
    WORD magic = 0;
    int has_nt = 0, fopen_ok = 0;

    full[0] = 0;
    sdmc[0] = 0;

    if (wm && wm->ldr.FullDllName.Buffer)
    {
        flen = wm->ldr.FullDllName.Length / sizeof(WCHAR);
        for (i = 0; i < flen && i + 1 < sizeof(full); i++)
        {
            WCHAR ch = wm->ldr.FullDllName.Buffer[i];
            full[i] = ch < 0x80 ? (char)ch : '?';
        }
        full[i] = 0;

        if (wine_nx_nt_name_to_sdmc_path( &wm->ldr.FullDllName, sdmc, sizeof(sdmc) ))
        {
            FILE *f = fopen( sdmc, "rb" );
            if (f) { fopen_ok = 1; fclose( f ); }
        }
        else sdmc[0] = 0;
    }

    if (base && (nt = RtlImageNtHeader( (HMODULE)base )))
    {
        has_nt = 1;
        magic  = nt->OptionalHeader.Magic;
        nrva   = nt->OptionalHeader.NumberOfRvaAndSizes;
        if (IMAGE_DIRECTORY_ENTRY_EXPORT < nrva)
        {
            exp_va = nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress;
            exp_sz = nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].Size;
        }
    }

    snprintf( wine_nx_export_diag, sizeof(wine_nx_export_diag),
              "%s base=%p nt=%d magic=0x%x nrva=%u exp_va=0x%x exp_sz=0x%x fopen=%d full=%s sdmc=%s",
              name ? name : "", base, has_nt, (unsigned)magic, (unsigned)nrva,
              (unsigned)exp_va, (unsigned)exp_sz, fopen_ok,
              full[0] ? full : "-", sdmc[0] ? sdmc : "-" );
}
#endif
...
static BOOL import_dll( WINE_MODREF *wm, const IMAGE_IMPORT_DESCRIPTOR *descr, LPCWSTR load_path, WINE_MODREF **pwm )
{
    ...
    BOOL iat_unprotected = FALSE;
#ifdef __SWITCH__
    struct wine_nx_file_exports file_exports;
    BOOL file_exports_opened = FALSE;
#endif
...
#ifdef __SWITCH__
    if (wine_nx_is_crt_apiset_name( name ))
    {
        wine_nx_trace( "[TR] api-set %s -> ucrtbase.dll", name );
        name = "ucrtbase.dll";
        len = strlen(name);
    }
#endif
...
#ifdef __SWITCH__
    {
        ...
        wine_nx_copy_ascii_wstr( buffer, wcslen(buffer), target, sizeof(target), FALSE );
        wine_nx_trace( "[TR] import %s -> %s build=%08x", name, target, (unsigned)status );
    }
#endif
    if (!status) status = load_dll( load_path, buffer, 0, &wmImp, system );
#ifdef __SWITCH__
    wine_nx_trace( "[TR] load_dll %s -> %08x", name, (unsigned)status );
#endif
    ...
    if (status)
    {
        ...
        if (status != STATUS_ENTRYPOINT_NOT_FOUND ||
            wine_nx_last_import_status != STATUS_ENTRYPOINT_NOT_FOUND)
            wine_nx_record_import_failure( name, status );
#endif
    ...
    {
#ifdef __SWITCH__
        if (!wine_nx_ensure_file_exports( wmImp, &file_exports, &file_exports_opened ))
        {
            wine_nx_record_export_diag( name, wmImp );
            wine_nx_record_export_failure( name, "<no-export-table>" );
            missing_export = TRUE;
            goto done;
        }
#else
     ...
     * readonly section. Keep this after export lookup on Switch, where
     * protection changes can remap image pages. */
    ...
    while (import_list->u1.Ordinal)
    {
        if (IMAGE_SNAP_BY_ORDINAL(import_list->u1.Ordinal))
        {
            ...
            if (exports)
                proc = find_ordinal_export( imp_mod, exports, exp_size,
                                            ordinal - exports->Base, load_path, wm, FALSE );
#ifdef __SWITCH__
            if (!proc && wine_nx_ensure_file_exports( wmImp, &file_exports, &file_exports_opened ))
            {
                char forward[256];
                DWORD file_rva;

                forward[0] = 0;
                if (wine_nx_file_find_ordinal_export( &file_exports, ordinal,
                                                      &file_rva, forward, sizeof(forward) ))
                {
                    if (forward[0])
                        proc = find_forwarded_export( imp_mod, forward, load_path, wm, FALSE );
                    else
                        proc = (FARPROC)((char *)imp_mod + file_rva);
                }
            }
            if (!proc)
            {
                ...
                snprintf( ordinal_name, sizeof(ordinal_name), "#%u", ordinal );
                wine_nx_trace( "[TR] missing export %s!%s", name, ordinal_name );
                wine_nx_record_export_failure( name, ordinal_name );
                missing_export = TRUE;
                break;
            }
#else
...
#endif
            else thunk_list->u1.Function = (ULONG_PTR)proc;
        ...
        else  /* import by name */
        {
            ...
            if (exports)
                proc = find_named_export( imp_mod, exports, exp_size, (const char*)pe_name->Name,
            ...
            if (!proc && wine_nx_ensure_file_exports( wmImp, &file_exports, &file_exports_opened ))
            {
                ...
                forward[0] = 0;
                if (wine_nx_file_find_named_export( &file_exports, (const char *)pe_name->Name,
                                                    pe_name->Hint, &file_rva, forward, sizeof(forward) ))
                {
                    if (forward[0])
                        proc = find_forwarded_export( imp_mod, forward, load_path, wm, FALSE );
                    else
            ...
            if (!proc)
            {
                wine_nx_trace( "[TR] missing export %s!%s", name, (const char *)pe_name->Name );
                wine_nx_record_export_failure( name, (const char *)pe_name->Name );
                missing_export = TRUE;
                break;
            }
#else
...
#ifdef __SWITCH__
    if (file_exports_opened) wine_nx_file_exports_close( &file_exports );
#endif
    if (missing_export) return FALSE;
    *pwm = wmImp;
    return TRUE;
}
...
static NTSTATUS fixup_imports( WINE_MODREF *wm, LPCWSTR load_path )
{
    ...
    {
        char nb[280];
        wine_nx_trace( "[TR] fixup_imports enter %s",
                       wine_nx_us_ascii( &wm->ldr.BaseDllName, nb, sizeof(nb) ) );
    }
#endif
    if (!(wm->ldr.Flags & LDR_DONT_RESOLVE_REFS)) return STATUS_SUCCESS;  /* already done */
    wm->ldr.Flags &= ~LDR_DONT_RESOLVE_REFS;
...
#ifdef __SWITCH__
    wine_nx_trace( "[TR] fixup_imports %d deps", nb_imports );
#endif
...
#ifdef __SWITCH__
            status = wine_nx_last_import_status ? wine_nx_last_import_status : STATUS_DLL_NOT_FOUND;
#else
...
static WINE_MODREF *alloc_module( HMODULE hModule, const UNICODE_STRING *nt_name, BOOL builtin )
{
    ...
    if ((tmp = wcsrchr( p, '/' ))) p = tmp + 1;
    RtlInitUnicodeString( &wm->ldr.FullDllName, buffer );
    RtlInitUnicodeString( &wm->ldr.BaseDllName, p );

#ifdef __SWITCH__
    {
        char base_name_ascii[128];
        unsigned int bni;
        for (bni = 0; bni < wm->ldr.BaseDllName.Length / sizeof(WCHAR) && bni < sizeof(base_name_ascii) - 1; bni++)
            base_name_ascii[bni] = wm->ldr.BaseDllName.Buffer[bni] < 0x80 ? (char)wm->ldr.BaseDllName.Buffer[bni] : '?';
        base_name_ascii[bni] = 0;
        wine_nx_trace( "[LDR] alloc_module basename='%s' len=%u base=%p",
                       base_name_ascii, (unsigned)(wm->ldr.BaseDllName.Length / sizeof(WCHAR)), wm->ldr.DllBase );
    }
    if (!wine_nx_pe_ntdll_module && !_wcsicmp( wm->ldr.BaseDllName.Buffer, L"ntdll.dll" ))
    {
        wine_nx_pe_ntdll_module = wm;
        wine_nx_trace( "[LDR] captured PE ntdll base=%p", wm->ldr.DllBase );
    }
#endif
...
static NTSTATUS process_attach( LDR_DDAG_NODE *node, LPVOID lpReserved )
{
    ...
    {
        char nb[128];
        wine_nx_copy_ascii_wstr( wm->ldr.BaseDllName.Buffer,
                                 wm->ldr.BaseDllName.Length / sizeof(WCHAR),
                                 nb, sizeof(nb), FALSE );
        wine_nx_trace( "[ATTACH] enter %s flags=0x%x entry=%p",
                       nb, (unsigned)wm->ldr.Flags, wm->ldr.EntryPoint );
    }
#endif
    ...
    if (status == STATUS_SUCCESS)
    {
...
#ifdef __SWITCH__
        {
            ...
            wine_nx_trace( "[ATTACH] calling DllMain %s entry=%p", nb, wm->ldr.EntryPoint );
        }
#endif
...
#ifdef __SWITCH__
        {
            ...
            wine_nx_copy_ascii_wstr( wm->ldr.BaseDllName.Buffer,
                                     wm->ldr.BaseDllName.Length / sizeof(WCHAR),
                                     nb, sizeof(nb), FALSE );
            wine_nx_trace( "[ATTACH] DllMain %s returned status=0x%08x", nb, (unsigned)status );
        }
#endif
        if (status == STATUS_SUCCESS)
        {
                              ...
                              DWORD flags, BOOL system, BOOL redirected, WINE_MODREF **pwm )
{
    ...
    {
        char nb[280];
        wine_nx_trace( "[TR] build_module %s base=%p", wine_nx_us_ascii( nt_name, nb, sizeof(nb) ), *module );
    }
#endif
...
#ifdef __SWITCH__
    wine_nx_trace( "[TR] relocs base=%p size=0x%lx", *module, (unsigned long)map_size );
#endif
    if ((status = perform_relocations( *module, nt, map_size ))) return status;
#ifdef __SWITCH__
    wine_nx_trace( "[TR] relocs done; fixup imports next" );
#endif
                               ...
                               SECTION_IMAGE_INFORMATION *image_info, struct file_id *id )
{
                              ...
                              FILE_SYNCHRONOUS_IO_NONALERT | FILE_NON_DIRECTORY_FILE )))
    {
        ...
        {
            char path[280];
            wine_nx_trace( "[TR] open %s -> %08x",
                           wine_nx_us_ascii( nt_name, path, sizeof(path) ), (unsigned)status );
        }
        wine_nx_record_open_status( nt_name, status );
#endif
    ...
    {
        char path[280];
        wine_nx_trace( "[TR] section %s -> %08x",
                       wine_nx_us_ascii( nt_name, path, sizeof(path) ), (unsigned)status );
    }
    wine_nx_record_open_status( nt_name, status );
#endif
    ...
    {
#ifdef __SWITCH__
        NTSTATUS qs = NtQuerySection( *mapping, SectionImageInformation, image_info,
                                      sizeof(*image_info), NULL );

        wine_nx_trace( "[TR] qsection->%08x machine=0x%x cur=0x%x contains_code=%d",
                       (unsigned)qs, image_info->Machine, current_machine,
                       image_info->ImageContainsCode );

        /* If the image info query did not succeed, image_info is unreliable
         * (uninitialised caller stack). Do NOT run is_valid_binary() on it or
         * hand it to NtMapViewOfSection() - that maps garbage and hangs. */
        if (qs)
        {
...
#ifdef __SWITCH__
static char wine_nx_ascii_lower( WCHAR ch )
{
    if (ch >= L'A' && ch <= L'Z') return (char)(ch - L'A' + L'a');
    if (ch == L'\\') return '/';
    if (ch < 0x80) return (char)ch;
...
static BOOL wine_nx_dos_dir_to_sdmc( const WCHAR *dos_dir, ULONG len, char *unix_dir, size_t size )
{
    const char *root;
    size_t out;
    ULONG i;
    ...
    out = strlen( root );
    if (out >= size) return FALSE;
    memcpy( unix_dir, root, out + 1 );

    for (i = 2; i < len; i++)
    {
        char ch = wine_nx_ascii_lower( dos_dir[i] );

        if (!ch) break;
        if (out + 1 >= size) return FALSE;
        if (ch == '/' && out && unix_dir[out - 1] == '/') continue;
        unix_dir[out++] = ch;
    }
    unix_dir[out] = 0;
    return TRUE;
...
static BOOL wine_nx_dll_has_ext( const char *name )
{
    size_t len = strlen( name );

    return len >= 4 && !strcmp( name + len - 4, ".dll" );
}

static BOOL wine_nx_normalize_dll_name( const WCHAR *dll, char *name, size_t size )
{
    const WCHAR *base = dll;
    const WCHAR *ptr;
    size_t out = 0;

    for (ptr = dll; *ptr; ptr++)
        if (*ptr == L'\\' || *ptr == L'/') base = ptr + 1;

    while (*base)
    {
        char ch = wine_nx_ascii_lower( *base++ );
    ...
    while (out >= 8 && !strcmp( name + out - 8, ".dll.dll" ))
    {
        out -= 4;
        name[out] = 0;
    }

    if (!wine_nx_dll_has_ext( name ))
    {
        if (out + 5 >= size) return FALSE;
        memcpy( name + out, ".dll", sizeof(".dll") );
    }
    return name[0] != 0;
}

static BOOL wine_nx_join_sdmc_dll_path( char *path, size_t size, const char *dir, const char *dll )
{
    size_t out = strlen( dir );

    if (out >= size) return FALSE;
    memcpy( path, dir, out + 1 );
    if (out && path[out - 1] != '/')
    {
        if (out + 1 >= size) return FALSE;
        path[out++] = '/';
...
static NTSTATUS wine_nx_build_unix_nt_name( const char *path, UNICODE_STRING *nt_name )
{
    static const WCHAR unix_prefix[] = {'\\','?','?','\\','u','n','i','x','/'};
    ULONG prefix_len = ARRAY_SIZE(unix_prefix);
    ULONG path_len = strlen( path );
    WCHAR *buffer;
    ULONG i;

    nt_name->Buffer = NULL;
    if (!(buffer = RtlAllocateHeap( GetProcessHeap(), 0, (prefix_len + path_len + 1) * sizeof(WCHAR) )))
        return STATUS_NO_MEMORY;

    memcpy( buffer, unix_prefix, sizeof(unix_prefix) );
    for (i = 0; i < path_len; i++) buffer[prefix_len + i] = (unsigned char)path[i];
    buffer[prefix_len + path_len] = 0;

    nt_name->Buffer = buffer;
    nt_name->Length = (prefix_len + path_len) * sizeof(WCHAR);
    nt_name->MaximumLength = nt_name->Length + sizeof(WCHAR);
    return STATUS_SUCCESS;
}

static NTSTATUS wine_nx_try_sdmc_dll_path( const char *unix_dir, const char *dll_name,
                                           ...
                                           SECTION_IMAGE_INFORMATION *image_info, struct file_id *id )
{
    ...
    if (!wine_nx_join_sdmc_dll_path( unix_path, sizeof(unix_path), unix_dir, dll_name ) ||
        wine_nx_build_unix_nt_name( unix_path, &candidate ))
        return STATUS_DLL_NOT_FOUND;

    status = open_dll_file( &candidate, pwm, mapping, image_info, id );
    if (status != STATUS_DLL_NOT_FOUND)
    {
        *nt_name = candidate;
        return status;
...
static NTSTATUS wine_nx_try_dos_dll_path( const WCHAR *dos_dir, ULONG len, const char *dll_name,
                                          UNICODE_STRING *nt_name, WINE_MODREF **pwm, HANDLE *mapping,
                                          SECTION_IMAGE_INFORMATION *image_info, struct file_id *id )
{
    static const WCHAR nt_prefix[] = {'\\','?','?','\\'};
    ULONG prefix_len = ARRAY_SIZE(nt_prefix);
    ULONG dll_len = strlen( dll_name );
    ULONG pos, i;
    BOOL need_sep;
    WCHAR *buffer;
    UNICODE_STRING candidate;
    ...
    if (len < 2 || dos_dir[1] != L':') return STATUS_DLL_NOT_FOUND;
    if (dos_dir[0] != L'c' && dos_dir[0] != L'C' && dos_dir[0] != L'z' && dos_dir[0] != L'Z')
        return STATUS_DLL_NOT_FOUND;

    while (len > 3 && (dos_dir[len - 1] == L'\\' || dos_dir[len - 1] == L'/')) len--;
    need_sep = !(len && (dos_dir[len - 1] == L'\\' || dos_dir[len - 1] == L'/'));

    if (!(buffer = RtlAllocateHeap( GetProcessHeap(), 0,
                                    (prefix_len + len + need_sep + dll_len + 1) * sizeof(WCHAR) )))
        return STATUS_NO_MEMORY;

    memcpy( buffer, nt_prefix, sizeof(nt_prefix) );
    pos = prefix_len;
    for (i = 0; i < len; i++)
    {
        WCHAR ch = dos_dir[i];
        buffer[pos++] = ch == L'/' ? L'\\' : ch;
    }
    if (need_sep) buffer[pos++] = L'\\';
    for (i = 0; i < dll_len; i++) buffer[pos++] = (unsigned char)dll_name[i];
    buffer[pos] = 0;

    candidate.Buffer = buffer;
    candidate.Length = pos * sizeof(WCHAR);
    candidate.MaximumLength = candidate.Length + sizeof(WCHAR);
...
static NTSTATUS wine_nx_search_sdmc_dll_file( const WCHAR *paths, const WCHAR *search, UNICODE_STRING *nt_name,
                                              WINE_MODREF **pwm, HANDLE *mapping,
                                              SECTION_IMAGE_INFORMATION *image_info, struct file_id *id )
{
    static const WCHAR dos_windows[] = {'C',':','\\','w','i','n','d','o','w','s',0};
    ...
    if (!wine_nx_normalize_dll_name( search, dll_name, sizeof(dll_name) ))
        return STATUS_DLL_NOT_FOUND;

    if (!paths) paths = default_load_path;
    while (paths && *paths)
    {
        const WCHAR *ptr = paths;
        char unix_dir[512];

        while (*ptr && *ptr != ';') ptr++;

        status = wine_nx_try_dos_dll_path( paths, ptr - paths, dll_name, nt_name, pwm, mapping, image_info, id );
        if (status != STATUS_DLL_NOT_FOUND)
            return status;

        if (wine_nx_dos_dir_to_sdmc( paths, ptr - paths, unix_dir, sizeof(unix_dir) ))
        {
            status = wine_nx_try_sdmc_dll_path( unix_dir, dll_name, nt_name, pwm, mapping, image_info, id );
            if (status != STATUS_DLL_NOT_FOUND)
                return status;
        }
    ...
    status = wine_nx_try_dos_dll_path( system_dir, ARRAY_SIZE(system_dir) - 1,
                                       dll_name, nt_name, pwm, mapping, image_info, id );
    if (status != STATUS_DLL_NOT_FOUND)
        return status;

    status = wine_nx_try_dos_dll_path( dos_windows, ARRAY_SIZE(dos_windows) - 1,
                                       dll_name, nt_name, pwm, mapping, image_info, id );
    if (status != STATUS_DLL_NOT_FOUND)
        return status;

    status = wine_nx_try_dos_dll_path( dos_root, ARRAY_SIZE(dos_root) - 1,
                                       dll_name, nt_name, pwm, mapping, image_info, id );
    if (status != STATUS_DLL_NOT_FOUND)
        return status;

    for (i = 0; i < ARRAY_SIZE(system_dirs); i++)
    {
        status = wine_nx_try_sdmc_dll_path( system_dirs[i], dll_name, nt_name, pwm, mapping, image_info, id );
        if (status != STATUS_DLL_NOT_FOUND)
            return status;
    }
    return status;
                                 ...
                                 DWORD flags, BOOL system, BOOL redirected, WINE_MODREF** pwm )
{
    ...
    {
        char nb[280];
        wine_nx_trace( "[TR] map_view %s", wine_nx_us_ascii( nt_name, nb, sizeof(nb) ) );
    }
#endif
    status = NtMapViewOfSection( mapping, NtCurrentProcess(), &module, 0, 0, NULL, &len,
                                 ViewShare, 0, PAGE_EXECUTE_READ );
#ifdef __SWITCH__
    wine_nx_trace( "[TR] map_view -> %08x base=%p", (unsigned)status, module );
#endif
...
static WINE_MODREF *wine_nx_find_ntdll_bruteforce(void)
{
    LIST_ENTRY *mark, *entry;
    LDR_DATA_TABLE_ENTRY *mod;
    static const WCHAR ntdll_name[] = {'n','t','d','l','l','.','d','l','l',0};

    mark = &NtCurrentTeb()->Peb->LdrData->InLoadOrderModuleList;
    for (entry = mark->Flink; entry != mark; entry = entry->Flink)
    {
        mod = CONTAINING_RECORD( entry, LDR_DATA_TABLE_ENTRY, InLoadOrderLinks );
        if (mod->BaseDllName.Length == 9 * sizeof(WCHAR) &&
            !RtlCompareUnicodeStrings( mod->BaseDllName.Buffer, mod->BaseDllName.Length / sizeof(WCHAR),
                                       ntdll_name, 9, TRUE ))
        {
            wine_nx_trace( "[LDR] brute-force found PE ntdll base=%p", mod->DllBase );
            return CONTAINING_RECORD( mod, WINE_MODREF, ldr );
        }
    }
    return NULL;
...
static void wine_nx_patch_ntdll_dispatchers(void)
{
    ...
    if (!ntdll) ntdll = find_basename_module( L"ntdll.dll" );
    if (!ntdll) ntdll = wine_nx_find_ntdll_bruteforce();
    if (!ntdll)
    {
        wine_nx_trace( "[LDR] PE ntdll not loaded yet; dispatcher patch deferred" );
        return;
    }

    wine_nx_trace( "[LDR] patching PE ntdll dispatchers; ntdll base=%p", ntdll->ldr.DllBase );

    syscall_dispatcher = RtlFindExportedRoutineByName( ntdll->ldr.DllBase, "__wine_syscall_dispatcher" );
    ...
    {
        wine_nx_trace( "[LDR] PE ntdll dispatcher exports missing syscall=%p unix=%p handle=%p teb=%p",
                       syscall_dispatcher, unix_call_dispatcher, unixlib_handle, pe_teb );
        return;
    }
    ...
    *unixlib_handle = __wine_unixlib_handle;
    ...
    wine_nx_trace( "[LDR] PE ntdll dispatchers patched syscall=%p unix=%p handle=%p teb=%p",
                   *syscall_dispatcher, *unix_call_dispatcher,
     ...
     * routes through the shim's malloc-backed RtlAllocateHeap, but real PE
     * modules (kernelbase, kernel32, ucrtbase) call into the real PE ntdll's
     * RtlAllocateHeap which dereferences the heap handle. Replace ProcessHeap
     * with a real heap created by the real PE ntdll, now that ntdll is loaded.
     *
     * The real PE ntdll accesses the TEB via x18 (ARM64 Windows convention).
     * Our unix-side ELF code doesn't keep x18 set to the TEB, so we wrap the
     * call: save x18, set x18 = NtCurrentTeb(), call, restore x18. */
    {
        PEB *peb = NtCurrentTeb()->Peb;
        void *real_create_heap = RtlFindExportedRoutineByName( ntdll->ldr.DllBase, "RtlCreateHeap" );
        if (peb && real_create_heap)
        {
            HANDLE real_heap = NULL;
            ...
            if (real_heap)
            {
                wine_nx_trace( "[LDR] swapping ProcessHeap shim=%p -> real=%p",
                               peb->ProcessHeap, real_heap );
                peb->ProcessHeap = real_heap;
            }
            else
            {
                wine_nx_trace( "[LDR] real RtlCreateHeap returned NULL; keeping shim heap %p",
                               peb->ProcessHeap );
            }
        }
        else
        {
            wine_nx_trace( "[LDR] could not resolve real RtlCreateHeap (peb=%p func=%p); keeping shim",
                           peb, real_create_heap );
        }
    }

    /* Initialize PE ntdll's `current_version` global. The standard LdrInitializeThunk
     * path calls version_init() during ntdll process attach, but we bypass that. Without
     * this, any RtlGetVersion / RtlVerifyVersionInfo call (e.g. from kernelbase via
        ...
        if (version_init_fn)
        {
            ...
            __asm__ volatile(
                ...
                : "x0", "x1", "x2", "x3", "x4", "x5", "x6", "x7", "x8", "x9",
                  "x10", "x11", "x12", "x13", "x14", "x15", "x16", "x17", "x18",
                  "x30", "memory", "cc" );
            wine_nx_trace( "[LDR] version_init() done" );
        }
        else
        {
            wine_nx_trace( "[LDR] version_init export not found in PE ntdll" );
        }
    }

    /* locale_init() normally runs from LdrInitializeThunk after the locale.nls
     * mapping is available.  The Switch bootstrap bypasses that entry point,
     * so initialize PE ntdll's private case/codepage tables explicitly. */
    {
        void *locale_init_fn = RtlFindExportedRoutineByName( ntdll->ldr.DllBase, "locale_init" );
        ...
        if (locale_init_fn)
        {
            ...
            __asm__ volatile(
                ...
                : "x0", "x1", "x2", "x3", "x4", "x5", "x6", "x7", "x8", "x9",
                  "x10", "x11", "x12", "x13", "x14", "x15", "x16", "x17", "x18",
                  "x30", "memory", "cc" );
            wine_nx_trace( "[LDR] locale_init() done" );
        }
        else
        {
            wine_nx_trace( "[LDR] locale_init export not found in PE ntdll" );
        }
    }
...
NTSTATUS wine_nx_loader_bootstrap( const UNICODE_STRING *main_nt_name )
{
    PEB *peb = NtCurrentTeb()->Peb;
    ULONG heap_flags = HEAP_GROWABLE;
    unsigned int i;

    if (wine_nx_loader_ready) return STATUS_SUCCESS;
    if (!peb || !peb->ProcessParameters || !peb->ImageBaseAddress || !main_nt_name)
        return STATUS_INVALID_PARAMETER;
    ...
    if (!peb->ProcessHeap) return STATUS_NO_MEMORY;
    ...
    wine_nx_main_module = wine_nx_build_main_module( main_nt_name );
    if (!wine_nx_main_module) return STATUS_INVALID_IMAGE_FORMAT;
...
NTSTATUS wine_nx_loader_fixup_main_imports(void)
{
    ...
    if (!wine_nx_loader_ready || !wine_nx_main_module) return STATUS_INVALID_PARAMETER;

    wine_nx_last_import_dll[0] = 0;
    wine_nx_last_import_status = STATUS_SUCCESS;
    RtlEnterCriticalSection( &loader_section );
    status = fixup_imports( wine_nx_main_module, NULL );
    if (!status) wine_nx_patch_ntdll_dispatchers();
    RtlLeaveCriticalSection( &loader_section );
    return status;
}

NTSTATUS wine_nx_loader_attach_main(void)
{
    static BOOL attach_done;
    WINE_MODREF *mod;
    ULONG_PTR cookie = 0;
    NTSTATUS status;

    if (!wine_nx_loader_ready || !wine_nx_main_module) return STATUS_INVALID_PARAMETER;
    if (attach_done) return STATUS_SUCCESS;

    RtlEnterCriticalSection( &loader_section );

    if (!node_ntdll && (mod = find_basename_module( L"ntdll.dll" )))
        node_ntdll = mod->ldr.DdagNode;
    if (!node_kernel32 && (mod = find_basename_module( L"kernel32.dll" )))
        node_kernel32 = mod->ldr.DdagNode;
    ...
    {
        attach_done = TRUE;
        wine_nx_main_module->ldr.Flags |= LDR_PROCESS_ATTACHED;
        if (wine_nx_main_module->ldr.ActivationContext)
            RtlActivateActivationContext( 0, wine_nx_main_module->ldr.ActivationContext, &cookie );
        wine_nx_trace( "[ATTACH] calling DLL_PROCESS_ATTACH for all modules" );
        status = walk_node_dependencies( wine_nx_main_module->ldr.DdagNode, (void *)1, process_attach );
        wine_nx_trace( "[ATTACH] DLL_PROCESS_ATTACH done status=0x%08x", (unsigned)status );
        if (!status && wine_nx_main_module->ldr.TlsIndex == -1)
            call_tls_callbacks( wine_nx_main_module->ldr.DllBase, DLL_PROCESS_ATTACH );
        if (wine_nx_main_module->ldr.ActivationContext)
            RtlDeactivateActivationContext( 0, cookie );
        if (status) attach_done = FALSE;
    }
                               ...
                               struct file_id *id, BOOL *redirected, BOOL find_loaded )
{
    ...
    *redirected = FALSE;
    ...
    {
        if (wine_nx_is_crt_apiset_dll( libname ))
        {
            static const WCHAR ucrtbase[] = L"ucrtbase.dll";

            TRACE( "redirecting %s to %s\n", debugstr_w(libname), debugstr_w(ucrtbase) );
            if ((*pwm = find_basename_module( ucrtbase ))) return STATUS_SUCCESS;
            libname = ucrtbase;
        }
    ...
    if (RtlDetermineDosPathNameType_U( libname ) == RtlPathTypeRelative)
    {
        ...
        {
            NTSTATUS sdmc_status = wine_nx_search_sdmc_dll_file( load_path, libname, nt_name, pwm,
                                                                 mapping, image_info, id );
            if (sdmc_status != STATUS_DLL_NOT_FOUND || status == STATUS_DLL_NOT_FOUND)
                status = sdmc_status;
        }
#endif
#ifndef __SWITCH__
                                             ...
                                             const UNICODE_STRING *libname, HMODULE* hModule)
{
    ...
    wine_nx_restore_pe_teb();
    wine_nx_trace( "[NXLDR] enter name=%s search=%p flags_ptr=%p module_ptr=%p teb=%p",
                   wine_nx_us_ascii( libname, namebuf, sizeof(namebuf) ), search_path,
                   load_flags, hModule, NtCurrentTeb() );
    dllname = append_dll_ext( libname->Buffer );

    if (load_flags) flags = *load_flags;
    if (!search_path || ((ULONG_PTR)search_path & 1))
    {
        nts = LdrGetDllPath( libname->Buffer, (ULONG_PTR)search_path & load_library_search_flags,
                             &path_name, &dummy );
        wine_nx_trace( "[NXLDR] path name=%s status=%08x path=%p",
                       namebuf, (unsigned int)nts, path_name );
        if (nts)
        {
    ...
    wine_nx_trace( "[NXLDR] load name=%s status=%08x wm=%p",
                   namebuf, (unsigned int)nts, wm );

    if (nts == STATUS_SUCCESS)
    {
        nts = process_attach( wm->ldr.DdagNode, NULL );
        wine_nx_trace( "[NXLDR] attach name=%s status=%08x base=%p",
    ...
    wine_nx_trace( "[NXLDR] return name=%s status=%08x module=%p",
                   namebuf, (unsigned int)nts, wm ? wm->ldr.DllBase : NULL );
    return nts;
}
                                       ...
                                       IMAGE_THUNK_DATA* addr, ULONG flags )
{
    ...
    wine_nx_trace( "[NXDELAY] enter base=%p desc=%p addr=%p flags=%08lx",
                   base, desc, addr, flags );
    ...
    id = addr - pIAT;
    wine_nx_trace( "[NXDELAY] dll=%.96s id=%ld phmod=%p current=%p",
                   name, (long)id, phmod, *phmod );
    ...
    if (!*phmod)
    {
        ...
        wine_nx_trace( "[NXDELAY] loading dll=%.96s", name );
        nts = LdrLoadDll(NULL, 0, &mod, phmod);
        RtlFreeUnicodeString(&mod);
        wine_nx_trace( "[NXDELAY] loaded dll=%.96s status=%08x module=%p",
                       name, (unsigned int)nts, *phmod );
        if (nts) goto fail;
    }

    if (IMAGE_SNAP_BY_ORDINAL(pINT[id].u1.Ordinal))
    {
        wine_nx_trace( "[NXDELAY] resolve dll=%.96s ordinal=%u", name,
    ...
    else
    {
        ...
        wine_nx_trace( "[NXDELAY] resolve dll=%.96s proc=%.96s", name, iibn->Name );
    ...
    if (!nts)
    {
        ...
        wine_nx_trace( "[NXDELAY] success dll=%.96s id=%ld proc=%p", name, (long)id, fp );
        return fp;
    }

fail:
    wine_nx_trace( "[NXDELAY] fail dll=%.96s id=%ld status=%08x dllhook=%p syshook=%p",
                   name, (long)id, (unsigned int)nts, dllhook, syshook );
