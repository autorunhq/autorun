...
#ifdef __SWITCH__
static ssize_t horizon_writev( int fd, const struct iovec *iov, int iovcnt )
{
    ssize_t total = 0;
    int i;
...
#else
#define wine_pthread_sigmask pthread_sigmask
#endif
...
unsigned int CDECL wine_server_call( void *req_ptr )
{
    ...
    wine_pthread_sigmask( SIG_BLOCK, &server_block_set, &old_set );
...
{
#ifdef __SWITCH__
    return horizon_server_receive_fd( handle );
#else
    struct iovec vec;
    struct msghdr msghdr;
    char cmsg_buffer[256];
...
#ifdef __SWITCH__
    u64 tid = 0;
    Result rc = svcGetThreadId( &tid, threadGetCurHandle() );

    if (R_SUCCEEDED(rc))
    {
        ret = (int)((tid ^ (tid >> 32)) & 0x7fffffff);
        if (!ret) ret = 1;
    }
...
void server_init_process( struct thread_data *data )
{
...
#ifdef __SWITCH__
    memset( &server_block_set, 0, sizeof(server_block_set) );
#else
...
#ifdef __SWITCH__
        fatal_error( "Horizon server returned no supported machines.\n" );
#else
    ...
    {
#ifdef __SWITCH__
        fatal_error( "Horizon server returned unsupported 32-bit machine %04x.\n", native_machine );
#else
        if (is_win64)
