...
NTSTATUS WINAPI NtTerminateProcess( HANDLE handle, LONG exit_code )
{
...
#ifdef __SWITCH__
    /* wineserver is stubbed on Horizon: terminate_process goes nowhere and
     * the caller (ucrtbase/_exit) retries forever. Log the exit code so we
     * can see why the process is bailing, then park. Returning through
    ...
    if (handle == GetCurrentProcess() || handle == NULL)
    {
        ...
        horizon_registry_flush();
        {
             ...
             * components setup). */
            extern unsigned int wine_nx_program_exit_code __attribute__((weak));

            if (&wine_nx_program_exit_code) wine_nx_program_exit_code = exit_code;
        }
    ...
    case ProcessBasicInformation:
        {
            ...
            if (size >= sizeof(PROCESS_BASIC_INFORMATION))
            {
                ...
                {
#ifdef __SWITCH__
                    /* wineserver is stubbed on Horizon: synthesize the reply from
                     * locals so DllMain doesn't get a NULL PebBaseAddress and fault. */
                    TEB *current_teb = NtCurrentTeb();

                    pbi.ExitStatus = STATUS_PENDING;
                    pbi.PebBaseAddress = peb ? peb : (current_teb ? current_teb->Peb : NULL);
                    pbi.AffinityMask = affinity_mask;
                    pbi.BasePriority = 8 /* NORMAL_PRIORITY_CLASS */;
                    pbi.UniqueProcessId = current_teb ?
                        (ULONG_PTR)current_teb->ClientId.UniqueProcess : 0;
    ...
    case ProcessTimes:
        {
            ...
            if (size >= sizeof(KERNEL_USER_TIMES))
            {
                ...
                {
#ifdef __SWITCH__
                    pti.UserTime.QuadPart = 0;
                    pti.KernelTime.QuadPart = 0;
#else
