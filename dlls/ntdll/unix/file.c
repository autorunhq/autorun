...
#ifdef __SWITCH__
static int horizon_asprintf( char **strp, const char *format, ... )
{
                                             ...
                                             BOOLEAN restart_scan, const UNICODE_STRING *mask )
{
    ...
    if (!status && sizeof(*entries) + total_len > length)
    ...
    {
        static int once;
        unsigned int info_size = dir_info_size( info_class, total_len / sizeof(WCHAR) );

        if (info_size > length) status = STATUS_INFO_LENGTH_MISMATCH;
        ...
        if (!status)
        {
            ...
            case FileDirectoryInformation:
            {
                FILE_DIRECTORY_INFORMATION *info = buffer;
                memset( info, 0, offsetof(FILE_DIRECTORY_INFORMATION, FileName) );
            ...
            case FileBothDirectoryInformation:
            {
                FILE_BOTH_DIRECTORY_INFORMATION *info = buffer;
                memset( info, 0, offsetof(FILE_BOTH_DIRECTORY_INFORMATION, FileName) );
                info->FileNameLength = entries->name_len;
                memcpy( info->FileName, entries + 1, entries->name_len );
                io->Information = offsetof(FILE_BOTH_DIRECTORY_INFORMATION, FileName) + total_len;
                TRACE( "-> %s.\n", debugstr_wn(info->FileName, info->FileNameLength / 2) );
                break;
            }
            case FileFullDirectoryInformation:
            {
                FILE_FULL_DIRECTORY_INFORMATION *info = buffer;
                memset( info, 0, offsetof(FILE_FULL_DIRECTORY_INFORMATION, FileName) );
                info->FileNameLength = entries->name_len;
                memcpy( info->FileName, entries + 1, entries->name_len );
                io->Information = offsetof(FILE_FULL_DIRECTORY_INFORMATION, FileName) + total_len;
                TRACE( "-> %s.\n", debugstr_wn(info->FileName, info->FileNameLength / 2) );
                break;
            }
            case FileNamesInformation:
            {
                FILE_NAMES_INFORMATION *info = buffer;
                memset( info, 0, offsetof(FILE_NAMES_INFORMATION, FileName) );
                info->FileNameLength = entries->name_len;
                memcpy( info->FileName, entries + 1, entries->name_len );
                io->Information = offsetof(FILE_NAMES_INFORMATION, FileName) + total_len;
                TRACE( "-> %s.\n", debugstr_wn(info->FileName, info->FileNameLength / 2) );
                break;
            }
            default:
                FIXME( "Unsupported info_class %d.\n", info_class );
                status = STATUS_NOT_SUPPORTED;
                break;
            }
        }
                                              ...
                                              BOOL open_reparse, unsigned int reparse_count )
{
    ...
    if (is_unix && name_len > prefix_len + 1 && (name[prefix_len] == '/' || name[prefix_len] == '\\'))
    {
        ...
        static const WCHAR romfsW[] = {'r','o','m','f','s',':'};
        const WCHAR *device_path = name + prefix_len + 1;
        unsigned int device_len = name_len - prefix_len - 1;

        if ((device_len > ARRAY_SIZE(sdmcW) && !wcsnicmp( device_path, sdmcW, ARRAY_SIZE(sdmcW) )) ||
            (device_len > ARRAY_SIZE(romfsW) && !wcsnicmp( device_path, romfsW, ARRAY_SIZE(romfsW) )))
        {
            unix_len = device_len * 3 + 1;
            if (!(unix_name = malloc( unix_len ))) return STATUS_NO_MEMORY;
            ret = ntdll_wcstoumbs( device_path, device_len, unix_name, unix_len - 1, TRUE );
            if (ret < 0 || ret >= unix_len - 1)
            {
                free( unix_name );
                return STATUS_OBJECT_NAME_INVALID;
            }
            unix_name[ret] = 0;
            for (pos = 0; unix_name[pos]; pos++) if (unix_name[pos] == '\\') unix_name[pos] = '/';
    ...
    if (prefix_len == 2 && prefix[1] == ':')
    {
        ...
        {
            free( unix_name );
            unix_len = name_len * 3 + strlen( drive_root ) + MAX_DIR_ENTRY_LEN + 3;
            if (!(unix_name = malloc( unix_len ))) return STATUS_NO_MEMORY;
            strcpy( unix_name, drive_root );
            pos = strlen( unix_name );
            ...
            prefix_len++;
            if (name_len > prefix_len && name[prefix_len] == '\\') prefix_len++;
            nt_pos += prefix_len;
                              ...
                              ULONG options, void *ea_buffer, ULONG ea_length )
{
...
#ifdef __SWITCH__
    {
            ...
            attr && attr->ObjectName)
        {
            ...
            unsigned int i, len = name->Length / sizeof(WCHAR);
            if (len > sizeof(ascii) - 1) len = sizeof(ascii) - 1;
            for (i = 0; i < len; i++)
            {
            ...
            ascii[len] = 0;
            snprintf( buf, sizeof(buf), "[NTOPEN] NtCreateFile name='%s' access=0x%x disp=%u options=0x%x",
                      ascii, (unsigned)access, (unsigned)disposition, (unsigned)options );
                                   ...
                                   PVOID out_buffer, UINT out_size )
{
    ...
    SERVER_START_REQ( ioctl )
    {
        ...
        if (wait_handle && status != STATUS_PENDING)
            set_sync_iosb( io, status, wine_server_reply_size( reply ), options );
#ifdef __SWITCH__
        /* the Horizon server completes ioctls synchronously without a wait
         * handle; the iosb must still be filled for callers that read it */
        else if (!wait_handle && status != STATUS_PENDING && !NT_ERROR(status))
            set_sync_iosb( io, status, wine_server_reply_size( reply ), options );
#endif
    }
    SERVER_END_REQ;
                                       ...
                                       void *out_buffer, ULONG out_size )
{
...
#ifdef __SWITCH__
    {
        ...
        if (&wine_nx_runtime_trace && &wine_nx_runtime_verbose && wine_nx_runtime_verbose)
        {
            ...
            switch (device)
            {
            case FILE_DEVICE_BEEP:           devname = "BEEP/AFD"; break;
            case FILE_DEVICE_NETWORK:        devname = "NETWORK"; break;
            case FILE_DEVICE_DISK:           devname = "DISK"; break;
            case FILE_DEVICE_CD_ROM:         devname = "CD_ROM"; break;
            case FILE_DEVICE_DVD:            devname = "DVD"; break;
            case FILE_DEVICE_CONTROLLER:     devname = "CONTROLLER"; break;
            case FILE_DEVICE_MASS_STORAGE:   devname = "MASS_STORAGE"; break;
            case FILE_DEVICE_SERIAL_PORT:    devname = "SERIAL"; break;
            case FILE_DEVICE_TAPE:           devname = "TAPE"; break;
            case FILE_DEVICE_NAMED_PIPE:     devname = "NAMED_PIPE"; break;
            case FILE_DEVICE_CONSOLE:        devname = "CONSOLE"; break;
            default: break;
            }
            snprintf( buf, sizeof(buf), "[IOCTL] NtDeviceIoControlFile handle=%p code=0x%08x device=0x%x(%s) in=%u out=%u",
                      handle, (unsigned)code, (unsigned)device, devname, (unsigned)in_size, (unsigned)out_size );
            wine_nx_runtime_trace( buf );
        }
    }
#endif
