...
static void get_initial_console( RTL_USER_PROCESS_PARAMETERS *params )
{
    ...
    {
#ifdef __SWITCH__
        params->dwXCountChars = 80;
        params->dwYCountChars = 25;
#else
        struct winsize size;
...
NTSTATUS WINAPI NtInitializeNlsFiles( void **ptr, LCID *lcid, LARGE_INTEGER *size )
{
    ...
    char trace_buf[256];
#define NLS_TRACE(fmt, ...) do { \
        if (&wine_nx_runtime_trace) { \
            snprintf( trace_buf, sizeof(trace_buf), fmt, ##__VA_ARGS__ ); \
            wine_nx_runtime_trace( trace_buf ); \
        } \
    } while (0)
#else
#define NLS_TRACE(...) do {} while (0)
#endif

    if (asprintf( &path, "%s/nls/locale.nls", dir ) == -1) return STATUS_NO_MEMORY;
    NLS_TRACE( "[NLS] Init: trying unix path=%s system_dir_prefix='\\??\\C:\\windows\\system32\\'", path );
    status = open_nls_data_file( path, system_dir, &file );
    NLS_TRACE( "[NLS] open_nls_data_file -> status=0x%08x file_handle=%p", (unsigned)status, file );
    free( path );
    if (!status)
    {
        status = NtCreateSection( &handle, SECTION_MAP_READ, NULL, NULL, PAGE_READONLY, SEC_COMMIT, file );
        NLS_TRACE( "[NLS] NtCreateSection -> status=0x%08x section=%p", (unsigned)status, handle );
        NtClose( file );
    }
    if (!status)
    {
        status = map_section( handle, ptr, &mapsize, PAGE_READONLY );
        NLS_TRACE( "[NLS] map_section -> status=0x%08x *ptr=%p mapsize=0x%lx", (unsigned)status, *ptr, (unsigned long)mapsize );
        NtClose( handle );
    }
    *lcid = system_lcid;
    NLS_TRACE( "[NLS] NtInitializeNlsFiles return status=0x%08x lcid=0x%08x", (unsigned)status, (unsigned)system_lcid );
#undef NLS_TRACE
    return status;
}
