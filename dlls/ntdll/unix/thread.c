...
void horizon_wait_suspend_arm64ec(void)
{
    ...
    if (!area || !area->ContextAmd64) return;
    area->ContextAmd64->ContextFlags = CONTEXT_AMD64_FULL;
    context_x64_to_arm_base( &context, area->ContextAmd64 );
    context.ContextFlags = CONTEXT_ARM64_FULL;
    if (area->SuspendDoorbell) __atomic_store_n( area->SuspendDoorbell, 0, __ATOMIC_RELEASE );
    wait_suspend( (CONTEXT *)&context );
    context_arm_to_x64_base( area->ContextAmd64, &context );
}
#endif
...
static BOOL is_current_thread_handle( HANDLE handle )
{
    THREAD_BASIC_INFORMATION info;

    if (handle == GetCurrentThread()) return TRUE;
    if (NtQueryInformationThread( handle, ThreadBasicInformation, &info, sizeof(info), NULL ))
        return FALSE;
    return HandleToULong( info.ClientId.UniqueThread ) == GetCurrentThreadId();
}
#endif
                                        ...
                                        const void *data, ULONG length )
{
    ...
    case ThreadAffinityMask:
    {
        ...
        SERVER_END_REQ;
#ifdef __SWITCH__
        if (!status && is_current_thread_handle( handle )) horizon_pin_current_thread( req_aff );
#endif
        return status;
    }
    ...
    case ThreadGroupInformation:
    {
        ...
        SERVER_END_REQ;
#ifdef __SWITCH__
        if (!status && is_current_thread_handle( handle )) horizon_pin_current_thread( req_aff->Mask );
#endif
        return status;
    }
