                     ...
                     UINT code, void *in_buffer, UINT in_size, void *out_buffer, UINT out_size )
{
     ...
     * field 4..15) so each call shows up identifiable in autorun_runtime.log. */
    {
        ...
        if (&wine_nx_runtime_trace && &wine_nx_runtime_verbose && wine_nx_runtime_verbose)
        {
            ...
            switch (code)
            {
            case IOCTL_AFD_BIND:                  name = "BIND"; break;
            case IOCTL_AFD_LISTEN:                name = "LISTEN"; break;
            case IOCTL_AFD_RECV:                  name = "RECV"; break;
            case IOCTL_AFD_POLL:                  name = "POLL"; break;
            case IOCTL_AFD_GETSOCKNAME:           name = "GETSOCKNAME"; break;
            case IOCTL_AFD_EVENT_SELECT:          name = "EVENT_SELECT"; break;
            case IOCTL_AFD_GET_EVENTS:            name = "GET_EVENTS"; break;
            case IOCTL_AFD_WINE_CREATE:           name = "WINE_CREATE"; break;
            case IOCTL_AFD_WINE_ACCEPT:           name = "WINE_ACCEPT"; break;
            case IOCTL_AFD_WINE_ACCEPT_INTO:      name = "WINE_ACCEPT_INTO"; break;
            case IOCTL_AFD_WINE_CONNECT:          name = "WINE_CONNECT"; break;
            case IOCTL_AFD_WINE_SHUTDOWN:         name = "WINE_SHUTDOWN"; break;
            case IOCTL_AFD_WINE_RECVMSG:          name = "WINE_RECVMSG"; break;
            case IOCTL_AFD_WINE_SENDMSG:          name = "WINE_SENDMSG"; break;
            case IOCTL_AFD_WINE_FIONBIO:          name = "WINE_FIONBIO"; break;
            case IOCTL_AFD_WINE_COMPLETE_ASYNC:   name = "WINE_COMPLETE_ASYNC"; break;
            case IOCTL_AFD_WINE_FIONREAD:         name = "WINE_FIONREAD"; break;
            case IOCTL_AFD_WINE_MESSAGE_SELECT:   name = "WINE_MESSAGE_SELECT"; break;
            case IOCTL_AFD_WINE_GETPEERNAME:      name = "WINE_GETPEERNAME"; break;
            default: break;
            }
            snprintf( buf, sizeof(buf), "[AFD] sock_ioctl handle=%p code=0x%08x (%s) in=%u out=%u",
                      handle, (unsigned)code, name, in_size, out_size );
